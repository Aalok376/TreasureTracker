...
const getProfilepic = async () => {
    try {
        ...
        ownprofile.innerHTML = profiles.map(profile => `
            <a class="profile two" href="profile.html" style="background-image: url('https://treasuretracker.onrender.com/${profile.user?.profilePicture?.replace(/\\/g, '/')}')"></a>
    ...
    if (OwnComments.length > 0 && OtherCommentsOnOwnPost.length === 0 && OtherCommentsOnOtherPost.length === 0) {
        return commentArea.innerHTML = OwnComments.map(comment => ` <div class="introareacomment" id="${comment._id}">
                       <div class="sectionforprofile">
                            <div class="profileimageforpost" data-user-id="${comment.userId._id}" style="background-image: url('https://treasuretracker.onrender.com/${comment.userId.profilePicture?.replace(/\\/g, '/')}')"></div>
