...
const getProfilepic = async () => {
    try {
        ...
        ownprofile.innerHTML = profiles.map(profile => `
                <div class="coverphoto" style="background-image: url('https://treasuretracker.onrender.com/${profile.user?.coverPicture?.replace(/\\/g, '/')}')"><div class="deletecoverimage"><i class="fa-solid fa-trash fa-2xl" style="color: #000000;"></i> </div></div>
                <div class="profilephoto" style="background-image: url('https://treasuretracker.onrender.com/${profile.user?.profilePicture?.replace(/\\/g, '/')}')"><div class="deleteprofileimage"><i class="fa-solid fa-trash fa-xl" style="color: #000000;"></i> </div></div>
                <div class="content">
        ...
        ownprofile2.innerHTML = profiles.map(profile => `
            <div class="profile" style="background-image: url('https://treasuretracker.onrender.com/${profile.user?.profilePicture?.replace(/\\/g, '/')}')"></div>
        `).join('')
    } catch (error) {
...
const getPost = async () => {
    try {
        const response = await fetch("https://treasuretracker.onrender.com/api/v1/getOwnPosts", {
            method: "GET",
            headers: {
...
ownprofile.addEventListener('click', async (event) => {
    ...
    const deletepp = event.target.closest('.deleteprofileimage')
    const deletecp = event.target.closest('.deletecoverimage')
    ...
    if (addpost) {
        ...
        try {
            (async () => {
                const response = await fetch(`https://treasuretracker.onrender.com/api/v1/deleteProfilePicture`, {
                    method: "DELETE"
                })
                const data = await response.json()
        ...
        try {
            (async () => {
                const response = await fetch(`https://treasuretracker.onrender.com/api/v1/deleteCoverPicture`, {
                    method: "DELETE"
                })
                const data = await response.json()
