...
const getNotifications = async () => {
    try {
        const response = await fetch("https://treasuretracker.onrender.com/api/v1/getNotification", {
            method: "GET",
            headers: {
        ...
        posthtml.innerHTML = Notificationss.map(post => `
            <div class="notificationcontainer" id=${post._id} data-user-id="${post.postId}">
                        <div class="sectionforprofile">
                            <div class="profileimagefornotification" style="background-image: url('https://treasuretracker.onrender.com/${post.senderId.profilePicture?.replace(/\\/g, '/')}')"></div>
...
const getele = async () => {
    ...
    const response = await fetch(`https://treasuretracker.onrender.com/api/v1/notificationsmarkAllAsRead`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
...
posthtml.addEventListener('click', async (event) => {
    ...
    if (event.target.closest('.cmakeasread')) {
        const response = await fetch(`https://treasuretracker.onrender.com/api/v1/notificationsread`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        try {
            (async () => {
                const response = await fetch(`https://treasuretracker.onrender.com/api/v1/deleteNotification/${notificationId}`, {
                    method: "DELETE"
                })
                const data = await response.json()
