...
const getotherProfilepic = async () => {
    try {
        const response = await fetch(`https://treasuretracker.onrender.com/api/v1/getotherprofile/${userId}`);
        const data = await response.json();

        profiles = Array.isArray(data) ? data : [data]

        ownprofile.innerHTML = profiles.map(profile => `
                <div class="coverphoto" style="background-image: url('https://treasuretracker.onrender.com/${profile.user?.coverPicture?.replace(/\\/g, '/')}')"></div>
                <div class="profilephoto" style="background-image: url('https://treasuretracker.onrender.com/${profile.user?.profilePicture?.replace(/\\/g, '/')}')"></div>
                <div class="content">
...
const getPost = async () => {
    try {
        const response = await fetch(`https://treasuretracker.onrender.com/api/v1//getotherposts/${userId}`, {
            method: "GET",
            headers: {
...
const friendStatus = async () => {
    ...
    try {
        ...
        const response = await fetch('https://treasuretracker.onrender.com/api/v1/getstatusoffriends', {
            method: 'POST',
            headers: {
                "Content-Type": "application/json",
            },
